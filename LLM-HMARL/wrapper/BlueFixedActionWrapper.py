...
class BlueFixedActionWrapper(BaseWrapper):
    ...
    def step(
        ...
        **kwargs : dict[str, Any]
        ...
        obs, rews, rews_org, dones, info = self.env.parallel_step(
            actions=action_dict, skill = skill, messages=messages, **kwargs
        )
        ...
        rewards_org = {
            agent: sum(reward.values())
            for agent, reward in rews_org.items()
            if "blue" in agent
        }
        ...
        return observations, rewards, rewards_org, terminated, truncated, info
